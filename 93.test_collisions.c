#include "hash_table_collision.h"
#include <stdio.h>
...
uint64_t hash_fnv1(char *text, size_t length) {
...
uint64_t hash_fnv0(char *text, size_t length) {
  uint64_t hash_value = 0;
  for(int i = 0; i < length; i++) {
    hash_value *= FNV_PRIME;
    hash_value ^= text[i];
...
int main(int argc, char **argv) {
  if(argc != 3) {
    printf("usage: %s <wordlist filetext> <num guesses>\n");
    return EXIT_FAILURE;
  }

  char *filetext = argv[1];
